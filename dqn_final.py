...
SAVE = True # True if you want to renew figure data, false otherwise
...
riseorfall = np.zeros((len(stock_data)-8,1))
for i in xrange(8,len(stock_data)):
	for j in xrange(i+1,len(stock_data)):
		if (float(stock_data[j].get('Close'))>=1.1*float(stock_data[i].get('Close'))):
			riseorfall[i-8] = 1
			break
		elif(float(stock_data[j].get('Close'))<=0.95*float(stock_data[i].get('Close'))):
			riseorfall[i-8] = 2
			break
print(len(K))

assert len(K) == len(riseorfall)

if SAVE:
	save_img(K, D, "2412_KD")

data_length = len(riseorfall)-DAY_LENGTH # discard the last image since no label is available

full_train_reward = 0
full_test_reward = 0
for i in xrange(len(train_data)):
	if riseorfall[i] == 1:
		full_train_reward += 10
	elif riseorfall[i] == 2:
		full_train_reward += 5
...
print("Full training reward is : ",full_train_reward)
print("Full test reward is: ",full_test_reward)
...
for i in xrange(len(train_data)):
	...
	train_label.append(riseorfall[i])
...
for i in xrange(len(train_data),data_length):
	...
	test_label.append(riseorfall[i])
	...
	def train_step(self,action): # for training, feed training data
		...
		if(self.stock_index)>= len(self.train_image)-1:
			stock_done = True
           		...
           		stock_done = False
		return self.train_image[self.stock_index], temp_reward, stock_done, 0
	...
	def test_step(self,action): # for testing, feed testing data
		...
		if(self.stock_index)>= len(self.test_image)-1:
			stock_done = True
           		...
           		stock_done = False
		return self.test_image[self.stock_index], temp_reward, stock_done, 0
		...
		for i in xrange(TEST):
			...
			for j in xrange(STEP):
				...
				if done:
					break
			if i == 0:
				print("Action 0: ",action0_count,". Action 1: ", action1_count)
		print()
