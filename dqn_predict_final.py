...
class TWStock():
	...
	def train_step(self,action): # For training, feed training data
		...
		action_reward = self.train_data[self.stock_index+5][DAY_LENGTH-1] - self.train_data[self.stock_index+1][DAY_LENGTH-1] # The reward is the close price for tomorrow minus today's
		if action == 0:
