...
flags.DEFINE_string("ps_hosts", "10.81.103.124:7777","List of hostname:port for ps jobs."
		    "This string should be the same on every host!!No blanks allowed between IPs!!!")
...
flags.DEFINE_integer("train_steps", 50,
                     "Number of training steps to perform, will start from previous ended step")
...
#air
    ...
    while True:
      ...
      if step >= FLAGS.train_steps+start_step:
        break
    ...
    ''' avg_acc = 0.0
    num_test_batch = 1000
    for i in xrange(int(10000/num_test_batch)):
        test_x, test_y = mnist.test.next_batch(num_test_batch)
        test_x, test_y = mnist.test.next_batch(num_test_batch)
  ...
  print(acc)
	...
	test_x, test_y = mnist.test.next_batch(1000)
       	#print(test_x.shape)
     	acc_batch = sess.run(accuracy, feed_dict={x: test_x, y_: test_y})
      	print(acc_batch)
  	acc_acu += acc_batch
    acc = acc_acu/10.0
    print("Test accuracy: %g"%acc)
#    print ("Test accuracy: %g"%avg_acc)#.eval(feed_dict={
